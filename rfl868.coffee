module.exports = (env) ->
  ...
  class RFL868Plugin extends env.plugins.Plugin
    ...
    init: (app, @framework, @config) =>
      ...
      @board.on("connected", () =>
        env.logger.info("Connected to RFL868 device.")
        if @config.rfdebug
      ...
      @board.on("error", (error) =>
        env.logger.error("Error with connection to RFL868 device: #{error.message}.")
...
#          return req.url.match(/^\/RFL868\/received.*$/)?
#        )
#        app.get('/RFL868/received', (req, res) =>
...
#        )
  ...
  class RFL868Switch extends env.devices.PowerSwitch
  ...
  class RFL868Dimmer extends env.devices.DimmerActuator
    _lastdimlevel: null
  ...
  class RFL868ContactSensor extends env.devices.ContactSensor
...
#  class RFL868Shutter extends env.devices.ShutterController
#
...
#
  ...
  class RFL868Pir extends env.devices.PresenceSensor
  ...
  class RFL868Data extends env.devices.Sensor
...
#  class RFL868GenericSensor extends env.devices.Sensor
#
...
#      if @attributes[name]?
#        throw new Error(
#          "Two attributes with the same name in RFL868GenericSensor config \"#{name}\""
#        )
  ...
  class RFEventPredicateProvider extends env.predicates.PredicateProvider
    ...
    parsePredicate: (input, context) ->  
      ...
      rfSwitchDevices = _(@framework.deviceManager.devices)
        .filter( (device) => device instanceof RFL868Switch ).value()
